...
test_that("Function str.tidyged() @ L231", {
  expect_snapshot_value(
   gedcom(subm("Me")) %>% 
    add_individual() %>% 
    add_individual() %>% 
...
test_that("Function df_individuals() @ L277", {
...
test_that("Function df_families() @ L335", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
   add_individual(sex = "M") %>% 
   add_individual_names("Joe /Bloggs/") %>% 
   add_individual(sex = "F") %>% 
...
test_that("Function df_multimedia() @ L369", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_sources() @ L396", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_repositories() @ L419", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_notes() @ L446", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
