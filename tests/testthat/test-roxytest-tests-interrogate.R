...
test_that("Function str.tidyged() @ L380", {
  expect_snapshot_value(
   gedcom(subm("Me")) %>% 
    add_individual() %>% 
    add_individual() %>% 
...
test_that("Function df_individuals() @ L426", {
...
test_that("Function df_families() @ L484", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
   add_individual(sex = "M") %>% 
   add_individual_names("Joe /Bloggs/") %>% 
   add_individual(sex = "F") %>% 
...
test_that("Function df_multimedia() @ L518", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_sources() @ L545", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_repositories() @ L568", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
...
test_that("Function df_notes() @ L595", {
  expect_snapshot_value(gedcom(subm("Me")) %>% 
