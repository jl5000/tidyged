---
title: "Naming individuals"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Naming individuals}
...
```
In the GEDCOM specification, all names should also be accompanied by a set of name pieces, specifying the given name, surname, nickname, etc. In addition, full names should always be surrounded surnames with forward slashes. In **tidygedcom** this allows us to automatically generate a name piece if none are given. If we provide a name with no name pieces and no forward slashes, it will cause an error:
...
GEDCOM files can also record different names *of the same type*, due to conflicting evidence. Below we define a name of Leia Skywaker (with no l) indicating some kind of transcription error:
