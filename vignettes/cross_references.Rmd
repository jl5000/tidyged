...
There are many other functions in the `gedcompendium` that take record xrefs as input parameters and it can be tedious to have to manually look these up. The `queryged` package offers a number of helper functions to locate specific xrefs using pattern matching:
