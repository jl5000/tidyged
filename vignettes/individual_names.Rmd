...
A shortcut way of defining a name is to use the `quick_name` parameter. It's not ideal, and should really only be reserved for demonstrations (like this one)! The parameter is the first one (after the tidyged object) which makes the code readable when using pipes:
...
Once we have defined a name for an individual, we can also define phonetic and romanised versions of the name. For this, we use the `add_indi_names_var()` function. As a minimum, the original name, variant name, derivation method, and whether it is a phonetic or romanised variation should be given:

```{r}
gedcom(subm("Me")) %>% 
  add_indi() %>% 
  add_indi_names("Leia /Skywalker/", type = "birth", given = "Leia", surname = "Skywalker") %>% 
  add_indi_names_var("Leia /Skywalker/", "Laya /Skywalker/", type = "As spoken",
...
gedcom(subm("Me")) %>% 
  ...
  add_indi_names_var("Leia /Skywalker/", "Laya /Skywalker/", type = "As spoken", 
                     phonetic_variation = TRUE, given = "Laya", surname = "Skywalker") %>% 
  add_indi_names("Leia /Organa/", type = "adoptive", given = "Leia", surname = "Organa") %>% 
...
gedcom(subm("Me")) %>% 
  ...
  add_indi_names_var("Leia /Skywalker/", "Laya /Skywalker/", type = "As spoken", 
                     phonetic_variation = TRUE, given = "Laya", surname = "Skywalker") %>% 
  add_indi_names("Leia /Organa/", type = "adoptive", given = "Leia", surname = "Organa") %>% 
  add_indi_names("Leia /Skywaker/", type = "birth", given = "Leia", surname = "Skywaker") %>%
  dplyr::filter(record == "@I1@") %>% print(n = Inf)
```
