...
You can see a summary of all facts associated with an individual with the `fact_summary_indi()` function.

```{r}
fact_summary_indi(dea_rel, "@I1@") %>% knitr::kable()
```
...
retro <- gedcom(subm("Me")) %>% 
  ...
  add_famg(husband = find_indi_name(., "Joe")) %>% 
  add_indi("F") %>% 
  add_indi_names(name_pieces(given = "Jess", surname = "Bloggs")) %>% 
  add_indi_links_to_families(spouse = find_indi_name(., "Joe"))
