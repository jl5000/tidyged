...
We illustrate its functionality using the family below consisting of three generations: A pair of grandparents who has a single child (named 'Parent'). This person forms two separate family groups with two spouses, each resulting in two children.

```{r}
three_gen <- gedcom(subm("Me")) %>% 
  add_individual() %>% 
  add_individual_names("Grandparent /1/") %>% 
  add_individual() %>% 
  add_individual_names("Grandparent /2/") %>% 
  add_individual() %>% 
  add_individual_names("Parent", given = "1") %>% 
  add_individual() %>% 
  ...
  add_family_group("Grandparent 1", "Grandparent 2", children = "Parent") %>% 
  add_family_group("Parent", "Spouse 1", children = c("Child 1", "Child 2")) %>%
  add_family_group("Parent", "Spouse 2", children = c("Child 1", "Child 2"))
...
```{r}
remove_descendants(three_gen, "Parent") %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Parent", remove_spouses = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Parent", remove_individual = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Parent", remove_individual = TRUE, remove_spouses = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
