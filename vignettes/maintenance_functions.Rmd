...
GEDCOM files can get very large and unwieldy, resulting in inefficiencies and file bloat. The `tidyged.utils` package offers functions to automate the cleaning of GEDCOM files.
...
```{r}
gedcom(subm("Me")) %>% 
  tidyged.utils::remove_change_dates() %>% 
  knitr::kable()
```
...
```{r}
tidyged.utils::consolidate_notes(notes) %>% 
  knitr::kable()
```
...
tidyged.utils::identify_unused_records(some_unref)
```
...
```{r}
remove_records(some_unref, tidyged.utils::identify_unused_records(some_unref)) %>% 
  str()
```
...
```{r}
tidyged.utils::identify_descendants(three_gen, "Main Person")
```
...
```{r}
three_gen %>% 
  tidyged.utils::identify_descendants("Main Person") %>% 
...
```{r}
three_gen %>% 
  tidyged.utils::identify_descendants("Main Person", include_spouses = TRUE) %>% 
  describe_records(three_gen, .)
```
...
```{r}
three_gen %>% 
  tidyged.utils::identify_descendants("Main Person", include_individual = TRUE) %>% 
  describe_records(three_gen, .)
```
...
```{r}
three_gen %>% 
  tidyged.utils::identify_descendants("Main Person", include_individual = TRUE, include_spouses = TRUE,
...
three_gen %>% 
  tidyged.utils::identify_descendants("Main Person", include_individual = TRUE, include_spouses = TRUE,
                                      include_families = TRUE) %>% 
  tidyged.utils::split_gedcom(three_gen, .) %>% 
  df_famg() %>% 
  knitr::kable()
```
