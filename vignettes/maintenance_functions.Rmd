...
some_unref <- gedcom(subm("Me")) %>% 
  add_individual("Tom /Smith/") %>% 
  add_individual("Tammy /Smith/") %>% 
  add_individual("Alice /White/") %>% 
  add_individual("Phil /Brown/") %>% 
  add_family_group(husband = "Tom", wife = "Tammy") %>% 
  add_family_group() %>% 
  add_family_group(husband = "Phil") %>% 
  add_family_group() %>% 
  add_family_group(children = "Alice") %>% 
  add_family_group() %>% 
  add_repository("Test repo") 
...
three_gen <- gedcom(subm("Me")) %>% 
  add_individual("Parent /1/") %>% 
  add_individual("Parent /2/") %>% 
  add_individual("Main /Person/") %>% 
  add_individual("Spouse /1/") %>% 
  add_individual("Spouse /2/") %>%
  add_individual("Child /1/") %>% 
  add_individual("Child /2/") %>% 
  add_individual("Child /3/") %>% 
  add_individual("Child /4/") %>% 
  add_family_group("Parent 1", "Parent 2", children = "Main Person") %>% 
  add_family_group("Main Person", "Spouse 1", children = c("Child 1", "Child 2")) %>%
  add_family_group("Main Person", "Spouse 2", children = c("Child 3", "Child 4"))

df_families(three_gen) %>% knitr::kable()
```

We use the `remove_descendants()` function below to remove the descendants of Main Person. By default it will keep the individual and their spouses, and will remove all empty family groups:

```{r}
remove_descendants(three_gen, "Main Person") %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Main Person", remove_spouses = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Main Person", remove_individual = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
...
```{r}
remove_descendants(three_gen, "Main Person", remove_individual = TRUE, remove_spouses = TRUE) %>% 
  df_families() %>% 
  knitr::kable()
```
