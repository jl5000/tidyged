...
We identify relations to an individual using the family of `get_*()` functions in the `queryged` package.
...
get_siblings(three_gen, find_indi_name(three_gen, "Child 1"), include_half_siblings = TRUE) %>% describe_records(gedcom = three_gen)
...
```{r}
three_gen %>% 
  get_descendants(find_indi_name(., "Main"), include_spouses = TRUE) %>% 
  describe_records(three_gen, .)
```
...
```{r}
three_gen %>% 
  get_descendants(find_indi_name(., "Main"), include_individual = TRUE) %>% 
  describe_records(three_gen, .)
```
...
```{r}
three_gen %>% 
  get_descendants(find_indi_name(., "Main"), include_families = TRUE) %>% 
  describe_records(three_gen, .)
```
...
```{r}
three_gen %>% 
  get_descendants(find_indi_name(., "Main"), include_individual = TRUE, include_spouses = TRUE,
