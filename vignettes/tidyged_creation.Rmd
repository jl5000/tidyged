...
You can also create a new `tidyged` object by subsetting an existing object. The `split_gedcom()` function from the `tidyged.utils` package subsets a tidyged object. This will retain the header and submitter information from the original object. 
...
This is especially powerful when paired with the `tidyged.utils::identify_descendants()` function (see `vignette("maintenance_functions")`), where you can create a separate object for an entire branch.
